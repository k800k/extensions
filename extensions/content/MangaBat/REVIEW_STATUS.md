...
- Live smoke (2026-07-17): Anonymous HTTPS landing page rendered HTML on `www.mangabats.com`; chapter images were observed on `img-r1.2xstorage.com` as WebP with a source referrer, with bounded fallback restricted to the reviewed `img-r2` and `imgs-2` mirrors. The probe discarded response bodies beyond browser rendering and used no account or cookies.
