...
This monorepo contains the MangaReader content/tracker Extension API 1.0/1.1 SDK, package CLI, catalog, and VitePress documentation app. The catalog contains 17 content packages and two tracker packages. Theme extension APIs are unsupported.
...
- `publish:dry-run` checks generated package and compatibility metadata without releasing anything.

## Publishing

The generated `dist/v1/stable/catalog.json` file is the website's built-in catalog source. Publishing must preserve stable unique IDs, increment package versions when executable bytes change, and retain source revisions, licenses, provenance, and compatibility metadata.
